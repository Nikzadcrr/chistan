...
func _build() -> void:
        ...
        if Save.unclaimed_achievements().size() > 0:
                ...
                var sb := UiKit.pill(UiKit.COL_RED, 11)
                dot.add_theme_stylebox_override("panel", sb)
                dot.position = Vector2(ach_btn.size.x - 6, -6)
...
func _maybe_daily_gift() -> void:
        ...
        claim.pressed.connect(func():
                ...
                tw.tween_property(veil, "color:a", 0.0, 0.3)
                tw.parallel().tween_property(panel, "modulate:a", 0.0, 0.3)
