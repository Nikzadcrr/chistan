...
func _ready() -> void:
	...
	deco.texture = load("res://assets/art/menu_bg.webp")
...
func add_player_row(list: VBoxContainer, idx: int) -> void:
	...
	_ensure_add_button(list)

func _ensure_add_button(list: VBoxContainer) -> void:
	## فقط یک دکمه «افزودن بازیکن» — همیشه آخر فهرست
	var add_row: HBoxContainer = list.get_node_or_null("AddRow")
	if add_row == null:
		add_row = HBoxContainer.new()
		add_row.name = "AddRow"
