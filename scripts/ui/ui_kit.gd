...
static func icon_path(name: String) -> String:
        return "res://assets/art/ic_%s.png" % name
...
static func stagger_in(items: Array, base_delay: float = 0.06, offset := Vector2(0, 60)) -> void:
        ...
        for i in items.size():
                ...
                tw.tween_property(c, "position", c.position - offset, 0.4).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT).set_delay(base_delay * i)
